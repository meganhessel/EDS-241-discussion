...
class_data <- read_csv("https://raw.github.com/garberadamc/W26-Policy-Eval/main/course-materials/labs/data/W26_class_survey.csv") %>% 
  mutate(random_groups = sample(rep(c("contol", "treatment"), each = n()/2)))
...
balance_summary %>% 
 ...
 tab_header(
  ...
  tab_spanner(
    ...
    columns = c(stat_1, stat_2) # extand over what columns ?
  ) %>% 
...
tbl_2way <- lobsters %>% 
  ...
  # Calucate total catch per unit effort for each year and region 
  group_by(year, region) %>% 
  summarize(
      total_cpue = sum(cpue, na.rm = TRUE),
            .groups = "drop") %>% # Same as `ungroup()`
  
  # pivot to create column for each 
  pivot_wider(
    names_from = region, 
...
tbl_3way <- 
  ...
  # pivot to create column for each treatment and control group within each region 
...
tbl_3way %>% 
  ...
  tab_header(
    title = "European Lobster Catch by Region, Year, Treatment Group", 
    subtitle = "Total Catch per Unit Effort (CPUE) by year, region, treatment group"
  ) %>% 
  tab_source_note(
    "Source: Moland et al., 2013"
  ) %>% 
...
table_w_plots <- lobsters %>% 
  # Caulcate total cpue for each year 
  group_by(year) %>% 
  summarise(
    total_cpue = sum(cpue, na.rm = TRUE), 
    dist_cpue = list(cpue), # store all indiviudla cpue values as a list for each year
    .groups = "drop"
  ) %>% 
  arrange(year) %>% 
  ...
  # Create gt table 
  gt() %>% 
  tab_header(title = "European Lobster CVatch Totals and Distribution  (2006-2010)", 
             subtitle = "Total Catch per Unit EFfort (CPUYE)") %>% 
  cols_label(year = "Year", 
             total_cpue = "Total CPUE", 
             dist_cpue = "Density CPUE"
             ) %>% 
  # Add in density plots 
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "dog"); beep(3)  
...
Non point Air pollution 

2. How were the treatment and control groups formed? 
44 neighborhoods - randomly put into group 
22 treatment: competited 
22 control 
