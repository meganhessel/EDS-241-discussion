...
*Response:*  distance from mangroves, Reef exposure (exposed), pollution risk (low) 
...
*Response:* Marine Protected Areas' are typically paced on or around Mangroves and reef habitats. These environments are nurseries, are a home to for various marine organisms, supply nutrients, offer protection. These ecosystem services increase fish biomass. 
...
*Response:* studying the average fish biomass at MPA (treated) sites. How fish biomass would change at MPAs. 
...
*Response:* 108 treated were matches (all treated recieved a match) and 37 control sites were matched to the treated (16 were not matched). 
...
Before and after shows effect of balancing, and evaluates how well the balancing algorithm matched the data. Optimizing balance on some of the controls can make the balance on others be "worse". 
...
reg_ecological <- lm(
  ...
  data = matched_data,
  weights = weights) # use weights in df 
...
*Response:* With treatment (in MPA sites), there is an increase of 80lbs of ecological biomass. Without treatment, there is expected to be an average of 106 biomass. The standard errors are huge! Thus, there is no statistical significance with the treatment vs no treatment. 

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* Matching does not help with unobservable differences. 
...
```{r}
mod1_nofe <- lm(perinc ~ vpd + I(vpd^2) + dbh + density, # no plot and year 
             data = data_fe)
...
*Response:* 0.126 -> 0.227 increased the coefficient - plot fixed effect was adding bias to the vapor pressure deficient levels. 
...
*Response:* Lots if rain or heat one year. El Nino.  
...
*Response:* density or close to stream. 
