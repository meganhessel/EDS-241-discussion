...
data_binned <- sim_data %>% 
  ...
  mutate(distance_bin = cut(distance, breaks = seq(-20, 20, by = 1), # Cut `distance` 
                            include.lowest = TRUE)) %>% # Include -20 
  # Calculate avg distance and deforestation for each bin
...
-   Lines represent deforestation to protected vs non protected areas. 
-   Trends reveal, as get closer to PA, deforestation rate decreases 
-   negative is outside the PA. Positive in inside the PA 
-   The discontinuity shows there is a PA effect 
...
-    Treatment effect: with treatment, deforestation rates decreases 0.1. Avg 10% decrease in deforestation. 
-    Interaction Term: With treatment, the change of deforestation over distance from border, increses by 0.002. Slope in protected area will be 0.002 higher than unprotected area. 
...
-   2.207 bandwidth estimate = 2.207 units to left and right
...
-   When limiting bandwidth, narrow sample size [-2,2] that is more centralized on treatment effect --> less observations but closer to the treatment effect --> increase comparability, increases variability (decrease statistical power). 

Account for ... weight (prioritize observations closer to the boundary), optimizing bandwidth, doesn't have to be linear (more accustom to data) (lm model has to be linear)

Originally using all of data 
...
rdd_poly <- rdplot(
  ...
  x.label = "Distance to PA Boarder (km)", 
  y.label = "Deforestation Rate"
)

# doesnt have to be linear - can follow trends 
```
...
**RESPONSE**:
-   Follows trends of data better, making prediction better. 
...
-   We are showing a little more discontinuity but very similar 
-   Our plot ratios seem to squish the relationships, making it harder to compare 
