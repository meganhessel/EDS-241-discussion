...
All states are different with different covariates –\> cofound
...
You want it near the same pre-treatment trends. If it does not track closely, cant interpret casual effect -\> the treatment isnt what causes the gap.
...
Include CO2, we are making sure we are matching the synthetic CO2 emissions pre treatment. Want to match pre treatment variables to find post-treatment variables.
...
scm_data <- data %>%
  # Running synethic control method for every state - 
  synthetic_control(
    outcome = co2_power_pc_tons, # setting the outcome varaible (per Capital CO2)
    unit = state, # unit identifier 
    time = year, # Time identifier 
    i_unit = "CA", # intervention unit / treated unit
    i_time = 2013, # intervetion for treated year 
   
    # Generaing placebos test for all states 
     generate_placebos = TRUE) %>% # How big are the gaps between TX and synthetci TX ? 
  
  # Defining characteristics synthetci CA mush match in order to be considered a good copy of real CA 
  
  generate_predictor(  
    time_window = 2000:2012, # pre treatement period only to compute predictors 
    coal_share = mean(coal_share, na.rm = TRUE), # avg share of electricty from coal
    income_pc = mean(income_pc, na.rm = TRUE), # Avg pre captial income 
    elec_price = mean(elec_price, na.rm = TRUE), # avg electricity price 
    co2_pre_mean = mean(co2_power_pc_tons, na.rm = TRUE)) %>% # avg pre treament outcome 
 # ^^ now evert state has 4 numbers that summarise pre-treatment characteritics, used to calculate weights 

  # Take the predictor summaries from above and figure out how much weight to give each donor state - what combo of donor states that represent CA 
...
Pre treatment - hovers around 0 (0.5, -0.5). Post has a large gap –\> treatment effect (expect this)
...
Does CA stand out pre and post treatment? - CA post treatment has a significant drop off.
...
CA = #1!! Big effect post treatment. Treatment effect = large. Post treatment divergence \>\> pre treatment.

CA pre treatment is the synethic model which assumes no volatility
...
Synthetic CA very similar to synthetic CA! However, there is varaition in elec_price  - hard to find an optimized that matched on all 4 variables - compromised to elec_price

2.  How does the synthetic control compare to the simple donor pool average?

Synthetic_CA = CA than donor_sample is to CA. Weighting did help!!
...
CA cap and trade is effective. The gap plot, placabo, and MSPE ratio show that the synthetic control method is viable and synthetic CA is a good counter factual.
...
2.  **Diagnostics checklist**: Review the four criteria from lecture — strong pre-treatment fit, transparent donor pool, no spillover, and favorable placebo comparison. How does our analysis perform on each?
    ...
    strong pre-treatment fit - yes 
    transparent donor pool - exluded the treated states 
    no spillover - yes, could be 
    favorable placebo comparison- yes 

plot_trends() show gaps with small pretreatment gaps and large post treatment gaps. 
plot_differences() 
...
Would see states with policy intervention differences. The estimate would strang towards 0 (not apples to apples anymore)
...
Here, we included pre-treatment avg as predictor (could have been large differences in pre treatment states)
