...
Air quality data for this analysis was obtained from the U.S. Environmental Protection Agency's (EPA) [Outdoor Air Quality Data](https://www.epa.gov/outdoor-air-quality-data/download-daily-data) portal. The dataset includes daily measurements of nitrogen dioxide (NO2) concentrations from monitoring stations in Los Angeles, California and Phoenix, Arizona.
...
# Read in data for LA and Phoenix 
...
#1 ASSUMPTION: PARALLE TRENDS (before intervention - control and treatment have paraelle / same trends before intervention) .... **VIOLATED** here. Cannot do Difference in Difference 
```{r}
# Paralle trend plot 
...
weekly_la_phx <- did_data_az_ca %>%
  mutate(
    date_obj = mdy(date),
    week = floor_date(date_obj, unit = "week") # Figure out week of observations and round to the Monday (floor_ceiling rounds up to Sunday)
  ) %>%
...
# create a line plot of phx and LA weekly NO2 levels 
ggplot(weekly_la_phx, aes(x = week, y = mean_no2, color = city)) + 
  geom_line(size = 1.2) + 
  geom_point(size = 2.5) + 
  theme_bw() + 
  
  # ADd CA lockdown state date 
  geom_vline(xintercept = as.Date("2020-03-19"), 
             linetype = "dashed", 
             color = "grey", 
             size = 1) + 
  # Add CA lockdown start date label 
  annotate("label", # "label = box; "text" = just words
           x = as.Date("2020-03-19"), 
           y = 35, 
           label = "CA Lockdown\nStarts", 
           size = 3) + 
  labs(
    title = "Weekly NO2 Trends in 2020: Los Angeles cs Phoenix", 
    y = "Weekly average NO2 (pph)", 
    x = "Date", 
    color = "City"
  ) + 
...
Parallel Trends Assumption = **ASSUMED** 
```{r}
#| message: FALSE
#| warning: FALSE
...
# Plot it! 
ggplot(did_weekly, 
       aes(x = week, y = mean_no2, color = state)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
...
```{r}
# Avg NO2 for obs in NW after Lockdown (march 23) and only for 2020
y_trt_post <- did_data_nm_tx %>% 
  filter(treated == 1, 
         is_post  == 1, 
         year == 2020, 
         month(date_obj) %in% c(2, 3, 4)) %>%
...
```
With treatment, there is a 3.5 reduction of NO2 ppb concentration. 
...
```{r}
tx_nm_did <- lm(daily_max_1_hour_no2_concentration ~ treated * is_post, # treated + is_post + treated:is_post
                 data = filter(did_data_nm_tx, 
                               year == 2020, 
                               month(date_obj) %in% c(2,3,4)))
...
What does this treatment effect of -3.516 mean? Talk with a partner!
:::
When comparing the 2 counties. the reduction of NO2 concentration with treatment is on average 3.5. However, this is not significantly significant. 
...
:::
Look at previous year NO2 concentration patterns.
...
Yes because parelle trends. But No. Texas population is 3 times as large. 
The data has violite movements (lots of ups and downs) and it over a short period of time. Therefore, it made it harder for DiD to spot different. 
...
-3.516, not statistically significant. Relationship between x and y is not strong enough to declare statistical significance. 22% prob that we would see these results *by chance.* 

--- 
3.  The DiD framework relies on a "Counterfactual"—the idea of what would have happened to New Mexico if it hadn't locked down. In this lab, what serves as our counterfactual? If a massive dust storm hit both El Paso and Dona Ana on March 25th, would that ruin our DiD estimate? Why or why not? (Hint: Think about whether the storm affects both groups equally).

Texas is the counterfactual. The counties are right nect to eachother in different states = experience similar conditions. A dust storm would hit both equally, so it would not ruin the DiD. 
...
Smaller. Because parallel trends! Mandate would have little effect. 
